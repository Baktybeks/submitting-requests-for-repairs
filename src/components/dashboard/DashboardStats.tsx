    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            {Object.entries(stats.requestsByCategory)
              ...
              .map(([category, count]) => (
                <CategoryBar
                  key={category}
                  label={getCategoryLabel(category as RequestCategory)}
                  count={count}
                  total={stats.totalRequests}
                  color="bg-indigo-500"
                />
              ))}
            {Object.values(stats.requestsByCategory).every(
              (count) => count === 0
            ) && (
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="space-y-3">
            {Object.entries(stats.requestsByPriority)
              .filter(([_, count]) => count > 0)
              .sort(([, a], [, b]) => b - a)
              .map(([priority, count]) => {
                const colors = {
                ...
                return (
                  <CategoryBar
                    key={priority}
                    label={getPriorityLabel(priority as RequestPriority)}
                    count={count}
                    total={stats.totalRequests}
            ...
            {Object.values(stats.requestsByPriority).every(
              (count) => count === 0
            ) && (
              <p className="text-gray-500 text-center py-4">
                Нет данных для отображения
